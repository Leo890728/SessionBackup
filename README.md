...
- 備份狀態比照檔案總管的 git 裝飾，以列尾字母表示（比對本機 manifest，不需連網）：
  ...
  - 已同步不顯示任何標記
- 圖示也在講狀態：未追蹤的專案是線框資料夾，備份庫裡已經有這個專案的對話時右下角
  多一顆雲（取消追蹤只是不再更新，既有備份仍在）；掃到疑似金鑰的對話會把對話圖示
  換成警告符號，備份與否仍由核取方塊與 U／M 各自表達。
- 點擊 session 可預覽 Markdown；可從預覽頁直接回到 Claude Code 或 Codex 的原生對話視窗。
- 預覽右緣有**提問清單**：平常只露出手把，滑過才滑出來，點手把可固定展開；點任一題
  捲到那則訊息，捲過之後標題列底下會貼一個「目前這一題」的小標籤，點它回到該則。
- 預覽開啟時停在最新訊息；下次備份會寫入的對話會有一條橫桿標出「已備份到哪裡」，
  位置取自已備份 revision 與現況的共同前綴，捲過之後會黏在標題列下方。
- 對話列尾（滑過才出現）有**在對話開啟**，直接跳回原生對話視窗；右鍵還可匯出
  Markdown、開啟原始 JSONL、納入或取消追蹤。
...
- 偵測到本機變更時，狀態磚下方會展開「**有變動的 sessions**」清單（類似 Source Control
  的 Changes）：以 **U**／**M** 標示新增或已變更，圖示是該對話所屬 AI 的標誌，名稱後方
  以次要文字顯示專案名稱，點擊可預覽對話；標題列的 `⋯` 可切換樹狀／清單顯示（樹狀依
  專案分組，選擇會記住），最多列 30 筆，其餘以「…還有 N 個」摘要。
...
- commit 前會掃描常見 Anthropic、OpenAI、GitHub、AWS、Slack、Google 金鑰與私鑰標頭；
  只掃這次會新寫入 store 的內容（沒變動的對話不掃，有變動的只掃新增的那幾行）。
